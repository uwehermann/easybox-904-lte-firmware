...
static int ppe_send(struct atm_vcc *vcc, struct sk_buff *skb)
{
  ...
  /*ctc*/
  #ifdef CONFIG_ARC_PPA_PKT_CAPTURE
	ebt_log_packet( skb, 10, 1, 7, vcc->vpi, vcc->vci);
  #endif
...
static int eth_qos_hard_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
  ...
  /*ctc*/
  #ifdef CONFIG_ARC_PPA_PKT_CAPTURE
	ebt_log_packet( skb, 0, 1, port, 0, 0 );
  #endif
...
static INLINE int dma_rx_int_handler(struct dma_device_info *dma_dev)
{
	...
	u32*	pPtr;
	struct Ehdr_1Q  *arc_ppa_ehdr = NULL;
	u8 bVlanPacket = 0;
  #endif
    ...
    case 1: //  MII1
		...
		#ifdef CONFIG_ARC_PPA_FILTER
		{
			...
			arc_ppa_ehdr=(struct Ehdr_1Q *)skb->data;
			if(arc_ppa_ehdr->tpid==htons(0x8100))
				bVlanPacket = 1;
	  ...
	  #ifdef CONFIG_ARC_PPA_PKT_CAPTURE
		ebt_log_packet( skb, 0, 0, off, 0, 0 );
	  #endif
        ...
        if ( (vcc = g_atm_priv_data.connection[conn].vcc) != NULL )
        {
            if ( atm_charge(vcc, skb->truesize) )
            {
                ...
                //else
                {
					...
					/* ctc */
				  #ifdef CONFIG_ARC_PPA_PKT_CAPTURE
					ebt_log_packet( skb, 10, 0, off, vcc->vpi, vcc->vci );
				  #endif
					/* ctc */
				  #ifdef CONFIG_ARC_PPA_FILTER
					if(bVlanPacket)
